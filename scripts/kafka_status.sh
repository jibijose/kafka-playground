...
#$KAFKA_HOME/bin/kafka-topics --list --bootstrap-server $HOST_IP:$KAFKA_PORT
#echo
echo  "All brokers list"
$KAFKA_HOME/bin/kafka-topics --describe --topic $TOPIC_NAME --bootstrap-server $HOST_IP:$KAFKA_PORT
echo

echo  "Brokers list with unavailable partitions"
$KAFKA_HOME/bin/kafka-topics --describe --topic $TOPIC_NAME --bootstrap-server $HOST_IP:$KAFKA_PORT --unavailable-partitions
echo

echo  "Brokers list with under replicated partitions"
$KAFKA_HOME/bin/kafka-topics --describe --topic $TOPIC_NAME --bootstrap-server $HOST_IP:$KAFKA_PORT --under-replicated-partitions
echo
